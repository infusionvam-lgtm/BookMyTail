...
const AdminBookings = () => {
  ...
  const { allBookings, status, error, metrics } = useSelector((state) => state.bookings);
  ...
  const totalRooms = useMemo(
    () =>
      allBookings.reduce(
        (acc, b) => acc + (b.rooms?.reduce((rAcc, r) => rAcc + (r.count || 1), 0) || 0),
        0
      ),
    [allBookings]
  );
  ...
  if (status === "loading")
    return <p className="pt-24 text-center text-white">Loading bookings...</p>;
