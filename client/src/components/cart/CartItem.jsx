...
const CartItem = ({ roomItem }) => {
  ...
  return (
    <li className="relative flex flex-col md:flex-row justify-between items-start md:items-center border rounded-lg p-3 m-3 mb-5">
            ...
            {lunchPrice > 0 && (
              <label className="flex items-center gap-1">
                <input
                  ...
                  onChange={(e) => handleUpdateRoom(roomId, roomItem.count, e.target.checked, roomItem.dinner)}
                />
                Lunch (₹{lunchPrice})
              </label>
            )}
            ...
            {dinnerPrice > 0 && (
              <label className="flex items-center gap-1">
                <input
                  ...
                  onChange={(e) => handleUpdateRoom(roomId, roomItem.count, roomItem.lunch, e.target.checked)}
                />
                Dinner (₹{dinnerPrice})
              </label>
            )}
      ...
      <div className="flex flex-row items-center md:items-center gap-4 mt-2 md:mt-0">
        ...
        <div className="flex items-center gap-4 border rounded-xl px-2 py-1">
          <button
            onClick={() => handleUpdateRoom(roomId, roomItem.count - 1, roomItem.lunch, roomItem.dinner)}
